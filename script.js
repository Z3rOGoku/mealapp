...
function updateBackgroundColor() {
    ...
    if (isWhite) {
        document.body.style.backgroundColor = "#618db8";
        navbar.style.backgroundColor = "white"; // Change navbar background color to white
        navbar.style.color = "black"; // Change navbar text color to black
        heading.style.color = "black"; // Change heading text color to black

        colorToggleButton.innerText = "DARK MODE 🌚";
    } else {
        document.body.style.backgroundColor = "rgb(36, 36, 36)";
        navbar.style.backgroundColor = "#007BFF"; // Change navbar background color back to default (blue in this example)
        navbar.style.color = "#fff"; // Change navbar text color back to default
        heading.style.color = "#fff"; // Change heading text color back to default
        colorToggleButton.innerText = "LIGHT MODE ☀️";
    }
}

//change emoji after a time
function changeEmoji() {
    const emojiElement = document.getElementById("emoji");
    const emojis = ["😋"," 🍴", "🍔"]; // Define the emojis you want to cycle through
    let currentIndex = 0;

    setInterval(function () {
        emojiElement.textContent = emojis[currentIndex];
        currentIndex = (currentIndex + 1) % emojis.length;
    }, 400); // Change the emoji every 0.4 second (adjust the interval as needed)
}

// Calling the function to start the emoji change animation
changeEmoji();